...
void DetectionManager::RegisterRGBController(RGBController *rgb_controller, const std::string& detection_path)
{
    ...
    /*-----------------------------------------------------*\
    | Detectors that register from inside their own body    |
    | never pass through RunDetectorWithTimeout's return    |
    | value, so fall back to the detector being run         |
    \*-----------------------------------------------------*/
    if(rgb_controller->detector_name.empty())
    {
        rgb_controller->detector_name = detection_string;
    }
...
void DetectionManager::BackgroundDetectI2CDevices(json& detector_settings)
{
    ...
    for(std::size_t i2c_detector_idx = 0; i2c_detector_idx < i2c_device_detectors.size(); i2c_detector_idx++)
    {
        ...
        if(this_device_enabled)
        {
            ...
            DetectedControllers detected_controllers = RunDetectorWithTimeout([&]() { return(i2c_device_detectors[i2c_detector_idx](i2c_buses)); }, detection_string.c_str(), DETECTOR_TIMEOUT_MS);
            ...
            for(std::size_t i2c_detector_idx = 0; i2c_detector_idx < i2c_dram_device_detectors.size(); i2c_detector_idx++)
            {
                if((i2c_dram_device_detectors[i2c_detector_idx].dram_type == dram_type) && is_jedec_in_slots(slots, i2c_dram_device_detectors[i2c_detector_idx].jedec_id))
                {
                    ...
                    if(this_device_enabled)
                    {
                        ...
                        DetectedControllers detected_controllers = RunDetectorWithTimeout([&]() { return(i2c_dram_device_detectors[i2c_detector_idx].function(i2c_buses[bus], matching_slots, i2c_dram_device_detectors[i2c_detector_idx].name)); }, detection_string.c_str(), DETECTOR_TIMEOUT_MS);
    ...
    for(std::size_t i2c_detector_idx = 0; i2c_detector_idx < i2c_pci_device_detectors.size(); i2c_detector_idx++)
    {
        ...
        if(this_device_enabled)
        {
            ...
            for(std::size_t bus = 0; bus < i2c_buses.size(); bus++)
            {
                ...
                {
                    DetectedControllers detected_controllers = RunDetectorWithTimeout([&]() { return(i2c_pci_device_detectors[i2c_detector_idx].function(i2c_buses[bus], i2c_pci_device_detectors[i2c_detector_idx].i2c_addr, i2c_pci_device_detectors[i2c_detector_idx].name)); }, detection_string.c_str(), DETECTOR_TIMEOUT_MS);
        ...
        while(current_hid_device)
        {
            if(detector.compare(current_hid_device))
            {
                ...
                if(this_device_enabled)
                {
                    ...
                    DetectedControllers detected_controllers = RunDetectorWithTimeout([&]() { return(detector.function(current_hid_device, hid_specific_detectors[hid_detector_idx].name)); }, detection_string.c_str(), DETECTOR_TIMEOUT_MS);
...
void DetectionManager::BackgroundDetectOtherDevices(json& detector_settings)
{
    ...
    for(std::size_t detector_idx = 0; detector_idx < device_detectors.size(); detector_idx++)
    {
        ...
        if(this_device_enabled)
        {
            ...
            DetectedControllers detected_controllers = RunDetectorWithTimeout([&]() { return(device_detectors[detector_idx]()); }, detection_string.c_str(), DETECTOR_TIMEOUT_MS);
        ...
        || (generic_detector && !skip_generic_detectors))
        {
            ...
            if(this_device_enabled)
            {
                ...
                if(detector->compare(current_hid_device))
                {
                    ...
                    DetectedControllers detected_controllers = RunDetectorWithTimeout([&]() { return(detector->function(current_hid_device, detector->name)); }, detection_string.c_str(), DETECTOR_TIMEOUT_MS);
        ...
        || (generic_detector && !skip_generic_detectors))
        {
            ...
            if(this_device_enabled)
            {
                ...
                if(detector->compare(current_hid_device))
                {
                    ...
                    DetectedControllers detected_controllers = RunDetectorWithTimeout([&]() { return(detector->function(*wrapper, current_hid_device, detector->name)); }, detection_string.c_str(), DETECTOR_TIMEOUT_MS);
...
{
    if(update_reason == DETECTIONMANAGER_UPDATE_REASON_RGBCONTROLLER_REGISTERED
    || update_reason == DETECTIONMANAGER_UPDATE_REASON_RGBCONTROLLER_UNREGISTERED)
    {
        WriteDetectorMap();
    }
...
/*---------------------------------------------------------*\
| Writes detector-map.json: every known controller's device |
| name mapped to the detector that produced it. The SDK     |
| device description carries no detector name, so a client  |
| that wants a device left alone cannot name the key it has |
| to write into the "Detectors" denylist, which is keyed by |
| detector. Table-driven detectors are where the two names  |
| diverge. See the README for the schema.                   |
|                                                           |
| Called on every controller list change, not just at the   |
| end of detection: on hotplug builds HID detection runs on |
| hidapi's event thread and lands after DetectDevices has   |
| already returned.                                         |
|                                                           |
| First writer wins on a duplicate device name. Nothing     |
| orders the writers, so a model reachable through two      |
| detectors can map to either one. Best-effort throughout:  |
| a failure here costs a client the mapping, never          |
| detection.                                                |
\*---------------------------------------------------------*/
void DetectionManager::WriteDetectorMap()
{
    ...
    {
        std::lock_guard<std::mutex> lock(RGBControllersMutex);
    ...
    const filesystem::path tmp_path = ResourceManager::get()->GetConfigurationDirectory() / "detector-map.json.tmp";

    /*-----------------------------------------------------*\
    | Written to a temp file and renamed over the real one: |
    | the reader is another process polling it, and a       |
    | truncating write in place hands it an empty document  |
    \*-----------------------------------------------------*/
    std::ofstream map_file(tmp_path, std::ios::out | std::ios::binary);

    if(!map_file)
    {
        LOG_WARNING("[%s] Cannot open %s for writing", DETECTIONMANAGER, tmp_path.string().c_str());
        return;
    }

    /*-----------------------------------------------------*\
    | Device names come from the hardware, and              |
    | wchar_to_char truncates each wchar_t to one byte, so  |
    | a non-ASCII USB product string is not valid UTF-8.    |
    | Strict dump() would throw on it and lose the whole    |
    | map                                                   |
    \*-----------------------------------------------------*/
    map_file << map_json.dump(4, ' ', false, json::error_handler_t::replace);
    map_file.close();

    if(!map_file)
    {
        LOG_WARNING("[%s] Cannot write detector map to %s", DETECTIONMANAGER, tmp_path.string().c_str());
        filesystem::remove(tmp_path);
        return;
    }

    std::error_code rename_error;
    filesystem::rename(tmp_path, map_path, rename_error);

    if(rename_error)
    {
        LOG_WARNING("[%s] Cannot replace detector map: %s", DETECTIONMANAGER, rename_error.message().c_str());
        filesystem::remove(tmp_path);
        return;
    }

    LOG_TRACE("[%s] Wrote detector map for %u device(s)", DETECTIONMANAGER, (unsigned int)map_json["devices"].size());
}
